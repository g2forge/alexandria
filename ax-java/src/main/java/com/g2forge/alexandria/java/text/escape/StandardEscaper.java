	...
	public void computeEscape(String string, final IConsumer1<? super TextUpdate<?>> consumer) {
		...
		for (int i = 0; i < string.length(); i++) {
			...
			if ((index >= 0) && ((required < 0) || (index < required))) {
				...
				consumer.accept(new TextUpdate<Object>(i, 1, escaped == null ? IFunction1.identity() : IFunction1.create(escaped.charAt(index))));
			...
			if ((prefix == null) || string.startsWith(prefix, i)) {
				...
				if (index >= 0) {
					if ((postfix == null) || string.startsWith(postfix, i + prefixLength + 1)) {
						...
						consumer.accept(new TextUpdate<Object>(i + prefixLength, 1, escaped == null ? IFunction1.identity() : IFunction1.create(unescaped.charAt(index))));
