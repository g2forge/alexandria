...
public class TestCache {
	...
	public void cache() {
		...
		Assert.assertEquals(1, cache.apply(0).intValue());
		Assert.assertEquals(Arrays.asList(0), function.getRecord().stream().map(ITuple2G_::get0).collect(Collectors.toList()));
	}
	...
	public void evict() {
		...
		Assert.assertEquals(Arrays.asList(0, 1), function.getRecord().stream().map(ITuple2G_::get0).collect(Collectors.toList()));
		Assert.assertEquals(1, cache.apply(0).intValue());
		Assert.assertEquals(Arrays.asList(0, 1, 0), function.getRecord().stream().map(ITuple2G_::get0).collect(Collectors.toList()));
	}
}
