 ...
 * 
 * @see HCommandLineBuilder#getCommandLineBuilder()
 */
public interface ICommandLineBuilder {
	/**
	 * Pre-process a command line before specifying it to {@link java.lang.ProcessBuilder#command(List)}. This is required in order to ensure OS specific
	 * quoting rules are properly applied. In short: the Java runtime has some truly horrifying bugs stemming from the fact that the API does not support the
