...
public class TODOMessageAnnotationHandler implements IAnnotationHandler<TODO> {
	...
	@Override
	public void handle(ProcessingEnvironment processingEnvironment, Element element, Supplier<String> path, Class<? extends TODO> annotationType, TODO... annotations) {
		...
		for (TODO annotation : annotations) {
			...
			if (annotation.deadline().length() > 0) {
				...
				try {
					...
					messager.printMessage(Diagnostic.Kind.ERROR, "TODO deadline \"" + annotation.deadline() + "\" could not be parsed: " + throwable, element);
				}
				error = _error;
			} else error = false;
			...
			messager.printMessage(error ? Diagnostic.Kind.ERROR : message.kind(), builder.toString(), element);
		}
	}
}
