...
public class JavaClassType extends AJavaType<Class<?>> implements IJavaClassType {
	...
	public Collection<? extends IJavaFieldType> getFields(final JavaMembership membership) {
		final Collection<IJavaFieldType> retVal = MapHelpers.map(new IMap1<Field, IJavaFieldType>() {
			@Override
			public IJavaFieldType map(final Field input) {
				return new JavaFieldType(input, environment);
			}
	...
	public List<? extends IJavaTypeVariable> getParameters() {
		return MapHelpers.map(new IMap1<TypeVariable<?>, IJavaTypeVariable>() {
			@Override
			public IJavaTypeVariable map(final TypeVariable<?> input) {
				return new JavaVariableType(input, environment);
			}
